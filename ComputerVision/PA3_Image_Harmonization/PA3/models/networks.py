...
def get_act_conv(act, dims_in, dims_out, kernel, stride, padding, bias, dropout_rate, dilation_rate, mode='reflect'):
    ...
    conv.extend([
        nn.Conv2d(
            ...
            dims_in, dims_out, kernel_size=kernel, stride=stride, dilation=dilation_rate, padding=padding, padding_mode=mode, bias=bias
        ),
...
def get_act_dconv(act, dims_in, dims_out, kernel, stride, padding, bias, dropout_rate=0.5):
    conv = [act]
    conv.extend([
...
class RainNet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=RAIN,
        ...
        self.layer4 = get_act_dconv(nn.ReLU(), 16*ngf, 4*ngf, 8, 2, 3, False)
        self.layer4RAIN = self.norm2(4*ngf)

        self.layer5 = get_act_dconv(nn.ReLU(), 8*ngf, 2*ngf, 8, 2, 3, False)
