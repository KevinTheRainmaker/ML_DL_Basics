...
from models.networks1 import RainNet
from models.normalize import RAIN
from data.dataset import Iharmony4Dataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_root', default='none', required=False,
                        type=str, help='the path of the dataset for evaluation')
    parser.add_argument('--batch_size', default=16,
                        required=False, type=int, help='batchsize of inference')
    parser.add_argument('--save_dir', default='evaluate', required=False,
                        type=str, help='directory to save evaluating results')
    parser.add_argument('--store_image', action='store_true',
                        required=False, help='whether store the result images')
    parser.add_argument('--device', default='cuda', type=str,
                        help='device to running the code | default cuda')
    ...
    assert device.startswith('cpu') or device.startswith(
        'cuda'), 'Device setting error. Please check --device in the arguments'

    testdata = Iharmony4Dataset(cfg, is_for_train=False)
    testloader = DataLoader(
        testdata, batch_size=user_args.batch_size, shuffle=False, drop_last=False)
    net = load_network(cfg)
    net = net.to(device)
    net.eval()

    os.makedirs(user_args.save_dir, exist_ok=True)
    fsave_results = open(os.path.join(
        user_args.save_dir, 'test_results.csv'), 'w')
    ...
    for i, batch_data in enumerate(tqdm(testloader)):
        ...
        for img_idx in range(comp.size(0)):
            ...
            # input shape should be (1, 3, H, W) or (1, 1, H, W)
            comp_rgb = util.tensor2im(comp[img_idx:img_idx+1])
        ...
        if i+1 % 50 == 0:
            fsave_results.flush()
    print('PSNR: %.4f  MSE: %.4f' %
          (calculateMean(all_psnr), calculateMean(all_mse)))
    fsave_results.flush()
    fsave_results.close()
