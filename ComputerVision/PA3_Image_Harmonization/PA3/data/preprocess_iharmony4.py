...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir_iharmony4', type=str, required=True,
                        help='data directory to iHarmony4 dataset')
    parser.add_argument('--save_dir', type=str, default='none',
                        help='data directory to save the resized images. Default none')
    parser.add_argument('--image_size', type=int, default=512,
                        help='image size to save in the local device. Default 512')
    parser.add_argument('--keep_aspect_ratio', action='store_true',
                        help='if keep the height-width aspect ratio unchanged. Default False')
    ...
    if args.save_dir == 'none':
        save_path = os.path.join(os.path.split(
            data_root)[0], 'iHarmony4Resized')
    else:
        save_path = args.save_dir
    os.makedirs(save_path, exist_ok=True)
    os.system('cp %s %s' %
              (os.path.join(args.dir_iharmony4, 'IHD_*'), save_path))
    ...
    for item in sub_dataset:
        ...
        os.makedirs(os.path.join(
            i_save_path, 'composite_images'), exist_ok=True)
    ...
    for running_list in [train_list, test_list]:
        for item in tqdm(running_list):
            ...
            if keep_aspect_ratio:
                # the new image keep its width-height ratio
            ...
            # resize the image
            comp = cv2.resize(
                comp, (new_size[1], new_size[0]), cv2.INTER_CUBIC)
            mask = cv2.imread(os.path.join(data_root, mask_path))
            mask = cv2.resize(
                mask, (new_size[1], new_size[0]), cv2.INTER_NEAREST)
            real = cv2.imread(os.path.join(data_root, real_path))
            real = cv2.resize(
                real, (new_size[1], new_size[0]), cv2.INTER_CUBIC)
