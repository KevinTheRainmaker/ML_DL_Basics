...
class Iharmony4Dataset(data.Dataset):
    ...
    def __init__(self, opt, is_for_train):
        ...
        self.transform = custom_transform(opt)
        self.opt = opt
    ...
    def _load_images_paths(self):
        if self.isTrain == True:
            ...
            self.trainfile = os.path.join(
                self.opt.dataset_root, 'IHD_train.txt')
            with open(self.trainfile, 'r') as f:
                for line in f.readlines():
                    ...
                    self.image_paths.append(
                        os.path.join(self.opt.dataset_root, line))
                    self.mask_paths.append(os.path.join(
                        self.opt.dataset_root, mask_path))
                    self.gt_paths.append(os.path.join(
                        self.opt.dataset_root, gt_path))

        elif self.isTrain == False:
            print('loading test file...')
            self.trainfile = os.path.join(
                self.opt.dataset_root, 'IHD_test.txt')
            with open(self.trainfile, 'r') as f:
                for line in f.readlines():
                    line = line.rstrip()
                    name_parts = line.split('_')
